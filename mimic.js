...
detector.addEventListener("onImageResultsSuccess", function(faces, image, timestamp) {
    ...
    if (faces.length > 0) {
        ...
        var corner = drawFeaturePoints(canvas, image, faces[0]);
        drawEmoji(canvas, image, faces[0], corner);
...
function drawFeaturePoints(canvas, img, face) {
    ...
    ctx.fillStyle = 'white';
    const radius = 5;
    var corner = {x: 0, y: 100000};
    // Loop over each feature point in the face
    for (var faceId in face.featurePoints) {
        var featurePoint = face.featurePoints[faceId];
        ctx.beginPath();
        ctx.arc(featurePoint.x, featurePoint.y, radius, 0, Math.PI * 2);
        ctx.stroke();
        if (featurePoint.x > corner.x) corner.x = featurePoint.x;
        if (featurePoint.y < corner.y) corner.y = featurePoint.y;
    }
    return corner;
}

// Draw the dominant emoji on the image
function drawEmoji(canvas, img, face, corner) {
    // Obtain a 2D context object to draw on the canvas
    var ctx = canvas.getContext('2d');
    ctx.font = '48px serif';
    ctx.fillText(face.emojis.dominantEmoji, corner.x, corner.y);
